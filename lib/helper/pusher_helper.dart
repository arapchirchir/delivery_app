...
class PusherHelper {
  ...
  static Future<void> initializePusher() async {
    PusherChannelsOptions testOptions = PusherChannelsOptions.fromHost(
      host:
          Get.find<SplashController>().configModel!.webSocketUri
              ?.replaceAll("wss://", "")
              .replaceAll("ws://", "") ??
          '',
      scheme:
          Get.find<SplashController>().configModel!.websocketScheme == 'https'
          ? 'wss'
          : 'ws',
      key: Get.find<SplashController>().configModel!.webSocketKey ?? '',
      port: int.parse(
        Get.find<SplashController>().configModel?.webSocketPort ?? '6001',
      ),
    );
    ...
    if (Get.find<SplashController>().pusherConnectionStatus != null ||
        ...
        authorizationDelegate:
            EndpointAuthorizableChannelTokenAuthorizationDelegate.forPrivateChannel(
              authorizationEndpoint: Uri.parse(
                '${AppConstants.baseUrl}${AppConstants.broadcastingAuthUrl}',
              ),
              headers: {
      ...
      if (driverTripSubscribe.currentStatus == null) {
        ...
        driverTripSubscribe.bind("customer-trip-request.$id").listen((event) {
          if (kDebugMode) {
            print(
              "Connected =====> Connection get from pusher > customer-trip-request",
            );
          }
          ...
          Get.find<RideController>().ongoingTripList().then((value) {
            if ((Get.find<RideController>().ongoingRide ?? []).isEmpty) {
              ...
              Get.find<RideController>().setRideId(
                jsonDecode(event.data!)['trip_id'].toString(),
              );
          ...
          customerInitialTripCancel(
            jsonDecode(event.data!)['trip_id'].toString(),
            id,
          );
          anotherDriverAcceptedTrip(
            jsonDecode(event.data!)['trip_id'].toString(),
            id,
          );
        });
      ...
      if (customerInitialTripCancelChannel.currentStatus == null) {
        ...
        customerInitialTripCancelChannel
            ...
            .listen((event) async {
              if (kDebugMode) {
                print(
                  "Connected =====> Connection get from pusher > customer-trip-cancelled",
                );
              }
              if (kDebugMode) {
                print(
                  "Connected =====> ${Get.find<RideController>().tripDetail?.id} > ${jsonDecode(event.data!)}",
                );
              }
      ...
      if (anotherDriverAcceptedTripChannel.currentStatus == null) {
        ...
        anotherDriverAcceptedTripChannel
            ...
            .listen((event) {
              if (kDebugMode) {
                print(
                  "Connected =====> Connection get from pusher > another-driver-trip-accepted",
                );
              }
    ...
    if (Get.find<SplashController>().pusherConnectionStatus != null ||
      ...
      if (tripCancelAfterOngoingChannel.currentStatus == null) {
        ...
        tripCancelAfterOngoingChannel
            ...
            .listen((event) {
              print(
                "Connected =====> Connection get from pusher > customer-trip-cancelled-after-ongoing",
              );

              Get.find<SafetyAlertController>().cancelDriverNeedSafetyStream();
              Get.offAll(
                () => RideDetailsScreen(
                  rideId: jsonDecode(event.data!)['id'].toString(),
                ),
              );
      ...
      if (tripPaymentSuccessfulChannel.currentStatus == null) {
        ...
        tripPaymentSuccessfulChannel
            ...
            .listen((event) {
              if (kDebugMode) {
                print(
                  "Connected =====> Connection get from pusher > customer-trip-payment-successful",
                );
              }

              Get.find<RideController>().ongoingTripList().then((value) {
                if ((Get.find<RideController>().ongoingRide ?? []).isEmpty) {
                  RideHelper.navigateToNextScreen(
                    id: jsonDecode(event.data!)['id'].toString(),
                  );
                } else {
  ...
  static void recordLocationViaPusher({
    ...
    if (Get.find<SplashController>().pusherConnectionStatus != null ||
        Get.find<SplashController>().pusherConnectionStatus == 'Connected') {
      final recordLocationChannel = pusherClient!.privateChannel(
      ...
      Future.delayed(Duration(seconds: 2), () {
        if (kDebugMode) {
          print(
            "<<<<-------- Location update via websocket ------->>>>> Lat: $latitude / Lon : $longitude",
          );
        }
